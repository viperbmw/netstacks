...
from netstacker.backend.core.security.whitelist import DeviceWhitelist
...
class ScrubFilter(logging.Filter):
    ...
    def scrub(self, message):
        lookup_table = [
            # {'library': <LibraryName.netmiko: 'netmiko'>, 'connection_args': {'device_type': 'cisco_ios', 'host': '10.0.2.24', 'username': 'admin', 'password': 'admin'}, 'command': 'show run | i hostname', 'args': {}, 'webhook': {}, 'queue_strategy': <QueueStrategy.pinned: 'pinned'>, 'post_checks': [], 'cache': {}}
            {
...
class Config:
    def __init__(self, config_filename=None, search_tfsm=True):
        ...
        self.netstacker_container_name = data["netstacker_container_name"]
        self.netstacker_callback_http_mode = data["netstacker_callback_http_mode"]
        ...
        for key in self.__dict__:  # Check for environment variables
            envvar_key = f"NETSTACKER_{key.upper()}"
    ...
    def find_actual_tfsm_path(self):
        potentials = [
            "netstacker/backend/plugins/extensibles/ntc-templates/index",
            "/code/netstacker/backend/plugins/extensibles/ntc-templates/index",
...
def initialize_config(search_tfsm: bool = True):
    return Config(os.getenv("NETSTACKER_CONFIG"), search_tfsm=search_tfsm)
