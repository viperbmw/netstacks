...
os.environ["NETSTACKER_CONFIG"] = str(ACTUAL_CONFIG_PATH)
from netstacker.backend.core.confload import confload


def test_netstacker_config_honors_envvar():
    with pytest.raises(KeyError):
        config = confload.Config("DOES NOT EXIST.json")
        _ = config.data["__comment__"]

    with pytest.raises(KeyError):
        os.environ["NETSTACKER_CONFIG"] = "DOES NOT EXIST.JSON"
    ...
    #     del os.environ["NETSTACKER_CONFIG"]    # but we're not doing that anymore and it's okay really
    #     config = confload.initialize_config()
...
def test_netstacker_config_value_precedence(monkeypatch):
    file_config = confload.Config(ACTUAL_CONFIG_PATH)
    monkeypatch.setenv("NETSTACKER_REDIS_SERVER", "123.COM")
...
def test_tfsm_search(monkeypatch):
    monkeypatch.setenv("NETSTACKER_TXTFSM_INDEX_FILE", "backend/plugins/extensibles/DOESNOTEXIT/index")
